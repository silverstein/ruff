            ...
            for key_map in all_key_maps {
                for (key, unpacked_key) in key_map {
                    result
                        .entry(key)
                        .and_modify(|existing| {
                            existing.value_ty = IntersectionBuilder::new(db)
                                .add_positive(existing.value_ty)
                                .add_positive(unpacked_key.value_ty)
                                .build();
...
fn validate_from_keywords<'db, 'ast>(
        ...
        .zip(unpacked_keyword_types.iter().copied())
    {
        ...
        if let Some(arg_name) = &keyword.arg {
            ...
            match guaranteed_keys.entry(arg_name.id.clone()) {
                Entry::Vacant(entry) => {
                    entry.insert(Some(keyword_node));
                }
            ...
            } else if let Some(unpacked_keys) = extract_unpacked_typed_dict_keys(db, unpacked_type)
            {
                for (key_name, unpacked_key) in &unpacked_keys {
                    if unpacked_key.is_required {
                        match guaranteed_keys.entry(key_name.clone()) {
                            Entry::Vacant(entry) => {
                                entry.insert(Some(keyword_node));
                            }
                            Entry::Occupied(mut entry) => match *entry.get() {
